...
class DsRuleUpdateRequest extends FormRequest
{
    ...
    {
        $id = $this->route('id') ?? $this->route('ds-rule');
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            if ($this->penyakit_id && $this->gejala_id) {
                // Get the ID from route parameter - the route uses {ds_rule}
                $id = $this->route('ds_rule');
