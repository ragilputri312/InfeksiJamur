				...
				<div class="wrap medium">
					<div class="page-intro">
						...
						<div class='gf_browser_safari gform_wrapper gform_legacy_markup_wrapper' id='gform_wrapper_1'
							...
							<form method='post' enctype='multipart/form-data' id='gform_1' action="{{ route('ds-diagnosis.process') }}" novalidate>
								@csrf
											...
											<ul id='gform_fields_1'
                                                ...
                                                <li id="field_{{ $loop->iteration }}"
                                                    ...
                                                    <label class='gfield_label'>
                                                        {{ $loop->iteration }}. {{ $item->pertanyaan ?? $item->gejala }}
                                                    ...
                                                    <div class='ginput_container ginput_container_radio'>
                                                        <ul class='gfield_radio' id='input_{{ $loop->iteration }}'>
                                                            ...
                                                            <li style='font-size: 14px;' class='gchoice gchoice_{{ $loop->iteration }}'>
                                                                <input
                                                                    name="answers[{{ $item->id }}]"
                                                                    type="radio"
                                                                    value="Ya"
                                                                    id="ya_{{ $item->id }}"
                                                                    onchange="updateAnswer({{ $item->id }}, 'Ya')" />
                                                                <label for="ya_{{ $item->id }}">Ya</label>
                                                            </li>
                                                            ...
                                                            <li style='font-size: 14px;' class='gchoice gchoice_{{ $loop->iteration }}'>
                                                                <input
                                                                    ...
                                                                    value="Tidak"
                                                                    id="tidak_{{ $item->id }}"
                                                                    onchange="updateAnswer({{ $item->id }}, 'Tidak')" />
                                                                <label for="tidak_{{ $item->id }}">Tidak</label>
                                                            </li>

                                                        </ul>

                                                        <!-- Dropdown untuk tingkat kemunculan (hanya muncul jika Ya dipilih) -->
                                                        <div id="kemunculan_{{ $item->id }}" class="kemunculan-dropdown" style="display: none; margin-top: 10px;">
                                                            <label for="kemunculan_select_{{ $item->id }}" style="font-size: 14px; font-weight: bold;">
                                                                Seberapa sering gejala ini muncul?
                                                            </label>
                                                            <select
                                                                name="kemunculan[{{ $item->id }}]"
                                                                id="kemunculan_select_{{ $item->id }}"
                                                                class="form-control"
                                                                style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px; font-size: 14px;">
                                                                <option value="">-- Pilih Tingkat Kemunculan --</option>
                                                                @foreach($kemunculanOptions ?? [] as $option)
...
let totalQuestions = {{ count($gejala) }};

function updateAnswer(gejalaId, value) {
	const kemunculanDiv = document.getElementById('kemunculan_' + gejalaId);
	const kemunculanSelect = document.getElementById('kemunculan_select_' + gejalaId);

	if (value === 'Ya') {
		answeredQuestions.add(gejalaId);
		kemunculanDiv.style.display = 'block';
		kemunculanSelect.required = true;
	} else {
		answeredQuestions.delete(gejalaId);
		kemunculanDiv.style.display = 'none';
		kemunculanSelect.required = false;
		kemunculanSelect.value = '';
	}
...
function updateSubmitButton() {
	...
	if (answeredCount >= 2) {
		...
		submitButton.textContent = 'Submit (' + answeredCount + ' gejala dipilih)';
		submitButton.style.opacity = '1';
	} else {
		submitButton.disabled = true;
		submitButton.textContent = 'Minimal pilih 2 gejala (' + answeredCount + '/2)';
...
document.getElementById('gform_1').addEventListener('submit', function(e) {
	if (answeredQuestions.size < 2) {
		e.preventDefault();
		alert('Silakan pilih minimal 2 gejala sebelum submit!');
		return false;
	}

	// Validasi tingkat kemunculan untuk gejala yang dipilih "Ya"
	for (let gejalaId of answeredQuestions) {
		const kemunculanSelect = document.getElementById('kemunculan_select_' + gejalaId);
		if (!kemunculanSelect.value) {
			e.preventDefault();
			alert('Silakan pilih tingkat kemunculan untuk semua gejala yang dipilih "Ya"!');
			return false;
		}
	}
});
